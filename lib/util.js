...
// Stable key for per-requester state (denial streak / cooldown), derived from
// the email so we never use the raw address as a document id.
...
// Base URL used to build owner-facing approve/deny links. NEVER derived from
// request headers (those are attacker-controlled and could redirect the owner's
// approval link to a server that steals the secret). Set PUBLIC_BASE_URL to the
// live site; falls back to the known production URL.
function publicBaseUrl() {
  var env = (process.env.PUBLIC_BASE_URL || "").trim().replace(/\/+$/, "");
  return env || "https://haider-portfolio-jet.vercel.app";
}

function clientIp(req) {
  // Prefer headers the platform sets itself (not client-spoofable). Fall back to
  // the rightmost x-forwarded-for entry (appended by Vercel), then the socket.
  var trusted = (req.headers["x-real-ip"] || req.headers["x-vercel-forwarded-for"] || "").split(",")[0].trim();
